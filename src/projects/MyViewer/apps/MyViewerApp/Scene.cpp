...
namespace sibr
{
	namespace DF_L
	{
		...
		BasicIBRScene::BasicIBRScene(const GaussianAppArgs& myArgs, SceneOptions myOpts)
		{
			BasicIBRScene();
			_currentOpts = myOpts;
			...
			DF_L::ParseData* pCastedParseData = dynamic_cast<DF_L::ParseData*>(_data.get());
			pCastedParseData->getParsedData(myArgs);
			std::cout << "Number of input Images to read: " << _data->imgInfos().size() << std::endl;

			if (_data->imgInfos().size() != _data->numCameras())
				SIBR_ERR << "List Image file size do not match number of input cameras in Bundle file!" << std::endl;

			if (pCastedParseData->Get_datasetType() != DF_L::Type::EMPTY) {
				createFromData(myArgs.texture_width);
			}
		}
