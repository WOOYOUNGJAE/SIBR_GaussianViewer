...
#include "projects/MyViewer/renderer/Config.hpp"
...
namespace sibr
{
	namespace DF_L
	{
		...
		void ParseData::getParsedOnlyPlyData(const std::string& dataset_path)
		{
		...
		void ParseData::getParsedData(const GaussianAppArgs& myArgs, const std::string& customPath)
		{
			// Only Ply -> gaussian normals already known
			std::string datasetTypeStr = myArgs.dataset_type.get();

			boost::algorithm::to_lower(datasetTypeStr);

			std::string onlyPlyPath = myArgs.modelPath.get() + "/OnlyPly/point_cloud.ply";
			if(sibr::fileExists(onlyPlyPath))
			{
				datasetTypeStr = "gaussian_with_normal";
				_datasetType = DF_L::Type::GAUSSIAN_WITH_NORMAL;
			}
			...
			if (datasetTypeStr == "gaussian_with_normal")
			{
				getParsedOnlyPlyData(myArgs.modelPath.get() + "/OnlyPly");
				goto PARSEDATA_FIN;
			}
			else if (datasetTypeStr == "sibr") {
				...
				_datasetType = DF_L::Type::COLMAP_CAPREAL;
			}
				...
				_datasetType = DF_L::Type::MESHROOM;
			}
			else if (datasetTypeStr == "blender")
			{
				...
				_datasetType = DF_L::Type::BLENDER;
			}
			else if (datasetTypeStr == "gaussian")
			{
				...
				{
					_datasetType = DF_L::Type::GAUSSIAN;
				}
					...
					_datasetType = DF_L::Type::COLMAP2;

				else if (sibr::fileExists(chunked))
				{
					_datasetType = DF_L::Type::CHUNKED;
				}
			...
			switch (_datasetType) {
			case DF_L::Type::GAUSSIAN:			getParsedGaussianData(myArgs.dataset_path); break;
			case DF_L::Type::BLENDER:			getParsedBlenderData(myArgs.dataset_path); break;
			case DF_L::Type::SIBR: 			getParsedBundlerData(myArgs.dataset_path, customPath, myArgs.scene_metadata_filename); break;
			case DF_L::Type::COLMAP_CAPREAL: getParsedColmapData(myArgs.dataset_path, myArgs.colmap_fovXfovY_flag, true); break;
			case DF_L::Type::COLMAP: 		getParsedColmapData(myArgs.dataset_path, myArgs.colmap_fovXfovY_flag, false); break;
			case DF_L::Type::COLMAP2: 		getParsedColmap2Data(myArgs.dataset_path, myArgs.colmap_fovXfovY_flag, false); break;
			case DF_L::Type::CHUNKED:			getParsedChunkedData(myArgs.dataset_path); break;
			case DF_L::Type::NVM: 			getParsedNVMData(myArgs.dataset_path, customPath, "/nvm/"); break;
			case DF_L::Type::MESHROOM: 		if (sibr::directoryExists(meshroom)) getParsedMeshroomData(myArgs.dataset_path.get() + "/../../");
							   else if (sibr::directoryExists(meshroom_sibr)) getParsedMeshroomData(myArgs.dataset_path); break;
			}

			PARSEDATA_FIN: // goto Tag for Only Ply Data
